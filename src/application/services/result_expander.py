...
from ...domain.models.season import HomogeneousZone
...
class ResultExpander:
    ...
    def expand(
        ...
        # Index of real results: (provider, pickup_date, rental_days) → BookingResult
        result_index: Dict[Tuple[str, date, int], BookingResult] = {}
        # Template per provider to build synthetic BookingSearch objects
        provider_templates: Dict[str, Tuple] = {}

        for search, res in pairs:
            if not res:
                continue
            pname = res.provider.name
            if res.cars:
        ...
        for provider_name, zones in provider_zones:
            if not zones or provider_name not in provider_templates:
                continue
            provider, pickup_loc, dropoff_loc = provider_templates[provider_name]
            ...
            while current <= period_end:
                for duration in sorted(durations):
                    ...
                    synthetic_res = BookingResult(provider=provider, cars=merged_cars)
