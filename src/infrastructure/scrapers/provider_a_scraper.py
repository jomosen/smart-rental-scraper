...
def _parse_daily_price(text: str) -> float:
    """Converts '33,99 € / Día' → 33.99"""
        ...
        for item in soup.select(".item-car"):
            ...
            for tarifa in item.select(".tarifa-item"):
                ...
                rates.append(Rate(
                    ...
                    currency="EUR",
                    total=float(tarifa.get("data-value", 0)),
                    daily_price=_parse_daily_price(per_day_el.get_text()) if per_day_el else 0.0,
                ))
