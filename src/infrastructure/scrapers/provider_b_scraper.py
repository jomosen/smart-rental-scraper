...
from ...domain.models.result import BookingResult, Car, Rate
...
def _parse_price(text: str) -> float:
    """Converts '32,86 € ', '(230,00 €)' or '1.100,04 €' → 32.86 / 230.0 / 1100.04"""
...
class ProviderBScraper(BaseScraper):
    ...
    async def _submit_form(self, criteria: BookingSearch) -> None:
        await self._driver.navigate(criteria.provider.base_url)
        await self._pause(2.0, 4.0)
        await self._fill_form(criteria)
    ...
    async def _extract_results(self, criteria: BookingSearch) -> BookingResult:
        ...
        for item in soup.select("div.package-template.ficha"):
            ...
            daily_price = _parse_price(daily_el.get_text()) if daily_el else 0.0
            total = _parse_price(total_el.get_text()) if total_el else 0.0
        ...
        return BookingResult(provider=criteria.provider, cars=cars)
