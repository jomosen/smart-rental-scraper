...
def create_log_dir(site_name: str, suffix: str = "_location") -> Path:
    """Create and return logs/<ts>_<site_name><suffix>/ with subdirs."""
    ts = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H-%M-%S")
    log_dir = _BASE_LOGS / f"{ts}_{site_name}{suffix}"
...
async def explore_location_field_in_session(
    ...
    """
    Assumes the page at *url* is already loaded in *session*.
    Runs steps 2-7: close cookies → capture form → identify fields → classify widget.
    All log events are appended to the caller's log_dir/trace.jsonl.
    """
    t0 = time.monotonic()
    total_cost = 0.0
    n_llm_calls = 0
...
async def explore_location_field(
    ...
    """
    Standalone entry point. Creates its own browser session, navigates to *url*,
    then delegates to explore_location_field_in_session.
    """
    ...
    async with BrowserSession(headless=headless) as session:
        ...
        return await explore_location_field_in_session(
            session, site_name, url, log_dir, client
        )
