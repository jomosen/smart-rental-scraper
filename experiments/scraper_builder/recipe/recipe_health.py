"""
Deterministic sanity checks for a recipe execution result.

check_recipe_health() evaluates the ScrapeResult returned by run_recipe()
without any LLM calls.  It distinguishes a broken recipe (wrong selectors,
stale markup) from a legitimate "no availability" result, and reports each
failed check with a human-readable reason.

Thresholds are module constants — parametrisable in Fase D.
"""
...
PRICE_PRESENCE_MIN = 0.80       # ≥80% of vehicles must have a price
KEY_FIELD_PRESENCE_MIN = 0.80   # ≥80% must have model and group_code
SOFT_FIELD_PRESENCE_MIN = 0.50  # transmission/seats — warn below this
PRICE_MIN = 5.0                 # wide range; only catches 0, negatives, absurdities
PRICE_MAX = 50_000.0
OUT_OF_RANGE_FAIL_FRACTION = 0.20   # >20% out-of-range → failed_check
...
def check_recipe_health(result) -> RecipeHealthCheck:
    """
    Evaluate the health of a recipe execution result without LLM.

    Distinguishes recipe breakage from a legitimate empty result:
    - 0 vehicles + page showed a no-availability message → healthy (warning)
    - 0 vehicles + content present but nothing extracted → broken

    Pure: no I/O, no browser.  Accepts any object with the same fields as
    ScrapeResult (duck typing so this module needn't import scraper_engine).
    """
    failed: list[str] = []
    warnings: list[str] = []

    # ── Check 1: phase reached ────────────────────────────────────────────────
    ...
    # ── Check 2: vehicle count vs. availability ───────────────────────────────
    if vehicle_count == 0:
    ...
    # Soft fields: warn but don't block
    ...
    # ── Check 4: prices in plausible range ───────────────────────────────────
