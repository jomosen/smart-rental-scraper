...
from src.application.smart_scraping.season_analyzer import SeasonAnalyzer
from src.domain.models.season import PricePoint


def _point(pickup: date, total: float, duration: int = 7, group: str = "A") -> PricePoint:
...
class TestNoChangeDetected:
    def test_single_zone_when_prices_are_stable(self):
        ...
        points = [
            _point(date(2026, 3, 1), 70.0),
            _point(date(2026, 3, 8), 71.0),
            _point(date(2026, 3, 15), 70.5),
        ]
    ...
    def test_no_price_points_returns_single_zone_with_zero_price(self):
        ...
        assert len(zones) == 1
        assert zones[0].reference_price == 0.0
        assert zones[0].start_date == START
        assert zones[0].end_date == END
...
class TestBoundaryDetection:
    def test_detects_single_price_jump(self):
        ...
        points = [
            _point(date(2026, 3, 1), 70.0),
            _point(date(2026, 3, 8), 70.0),
            _point(date(2026, 3, 15), 105.0),  # +50% → boundary
            _point(date(2026, 3, 22), 104.0),
        ]
        zones = analyzer.detect_zones(points, START, END, "A")
        assert len(zones) == 2
    ...
    def test_detects_two_boundaries(self):
        ...
        points = [
            _point(date(2026, 3, 1),  70.0),
            _point(date(2026, 3, 8),  140.0),  # +100%
            _point(date(2026, 3, 15), 70.0),   # −50%
        ]
        zones = analyzer.detect_zones(points, START, END, "A")
        assert len(zones) == 3
    ...
    def test_ignores_change_below_threshold(self):
        ...
        points = [
            _point(date(2026, 3, 1), 100.0),
            _point(date(2026, 3, 8), 105.0),  # +5% < 10% threshold
        ]
        zones = analyzer.detect_zones(points, START, END, "A")
        assert len(zones) == 1
...
class TestRepresentativeDate:
    ...
    def test_middle_strategy_uses_midpoint(self):
        ...
        points = [_point(date(2026, 3, 1), 70.0)]
        zones = analyzer.detect_zones(points, START, END, "A")
...
class TestGroupIsolation:
    def test_only_processes_matching_group(self):
        analyzer = SeasonAnalyzer(price_change_threshold=0.05)
        points = [
            _point(date(2026, 3, 1),  70.0, group="A"),
            _point(date(2026, 3, 8),  140.0, group="B"),  # price jump but group B
        ]
